...
3.  A [`shard`](/docs/v1.0/m3db/architecture/sharding) is effectively the same as a "virtual shard" in Cassandra in that it provides an arbitrary distribution of time series data via a simple hash of the series ID. A shard owns multiple `series`.
...
5. The `buffer` is where all data that has yet to be written to disk gets stored in memory, for a specific series. This includes both new writes to M3DB and data obtained through [bootstrapping](/docs/v1.0/operational_guide/bootstrapping_crash_recovery). More details on the [buffer](/docs/v1.0/m3db/architecture/engine#buffer) is explained below. Upon [flushing](/docs/v1.0/m3db/architecture/engine#flushing), the buffer creates a `block` of its data to be persisted to disk.
...
M3DB takes a two-pronged approach to persistent storage that involves combining a [commit log](/docs/v1.0/m3db/architecture/commitlogs) for disaster recovery with periodic flushing (writing [fileset files](/docs/v1.0/m3db/architecture/storage) to disk) for efficient retrieval:
...
2.  Periodically (based on the configured block size), all data in the buffer is flushed to disk as immutable [fileset files](/docs/v1.0/m3db/architecture/storage). These files are highly compressed and can be indexed into via their complementary index files. Check out the [flushing section](#flushing) to learn more about the background flushing process.
...
At the same time, the write will be appended to the commit log, which is periodically compacted via a snapshot process. Details of this is outlined in the [commit log](/docs/v1.0/m3db/architecture/commitlogs) page.

{{% notice warning %}}
Regardless of the success or failure of the write in a single node, the client will return a success or failure to the caller for the write based on the configured [consistency level](/docs/v1.0/m3db/architecture/consistencylevels).
{{% /notice %}}
...
3.  The block from disk (the block will be retrieved from disk and will then be cached according to the current [caching policy](/docs/v1.0/m3db/architecture/caching)) **[2PM - 4PM]**
...
Once M3DB has retrieved the three blocks from their respective locations in memory / on-disk, it will transmit all of the data back to the client. Whether or not the client returns a success to the caller for the read is dependent on the configured [consistency level](/docs/v1.0/m3db/architecture/consistencylevels).
...
As discussed in the [architecture](#architecture) section, writes are actively buffered / compressed in memory and the commit log is continuously being written to, but eventually data needs to be flushed to disk in the form of [fileset files](/docs/v1.0/m3db/architecture/storage) to facilitate efficient storage and retrieval.
...
Depending on the configured [caching policy](/docs/v1.0/m3db/architecture/caching), the [in-memory object layout](#in-memory-object-layout) can end up with references to series or data blocks that are expired (have fallen out of the retention period) or no longer needed to be in memory (due to the data being flushed to disk or no longer needing to be cached). The background tick will identify these structures and release them from memory.
