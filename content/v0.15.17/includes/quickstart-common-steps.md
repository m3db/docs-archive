...
The `namespaceName` argument must match the namespace in the `local` section of the `M3Coordinator` YAML configuration. If you [add any namespaces](/docs/v0.15.17/operational_guide/namespace_configuration) you also need to add them to the `local` section of `M3Coordinator`'s YAML configuration.
...
{{% notice tip %}}
[Read more about the bootstrapping process](/docs/v0.15.17/operational_guide/bootstrapping_crash_recovery/).
{{% /notice %}}
...
-   _[{{% apiendpoint %}}prom/remote/write](/docs/v0.15.17/m3coordinator/api/remote/)_ - Write a Prometheus remote write query to M3DB with a binary snappy compressed Prometheus WriteRequest protobuf message.
