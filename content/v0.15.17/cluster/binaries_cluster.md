...
This guide shows you the steps involved in creating an M3 cluster using M3 binaries, typically you would automate this with infrastructure as code tools such as Terraform or [Kubernetes](/docs/v0.15.17/operator).

{{% notice note %}}
This guide assumes you have read the [quickstart](/docs/v0.15.17/quickstart/binaries), and builds upon the concepts in that guide.
{{% /notice %}}
...
Depending on the default limits of your bare-metal machine or VM, M3 may need some Kernel tweaks to run as efficiently as possible, and [we recommend you review those](/docs/v0.15.17/operational_guide/kernel_configuration) before running M3 in production.
...
{{% notice tip %}}
You can find more information on configuring M3DB in the [operational guides section](/docs/v0.15.17/operational_guide/).
{{% /notice %}}
...
We recommend a replication factor of **3**, with each replica spread across failure domains such as a physical server rack, data center or availability zone. Read our [replication factor recommendations](/docs/v0.15.17/operational_guide/replication_and_deployment_in_zones) for more details.

### Shards

Read the [placement configuration guide](/docs/v0.15.17/operational_guide/placement_configuration) to determine the appropriate number of shards to specify.
