...
In addition, M3DB has support for automatically replicating data between isolated M3DB clusters (potentially running in different zones / regions). More details can be found in the [Replication between clusters](/docs/v1.1/operational_guide/replication_between_clusters) operational guide.

## Replication

A replication factor of at least 3 is highly recommended for any M3DB deployment, due to the consistency levels (for both reads and writes) that require quorum in order to complete an operation. For more information on consistency levels, see the documentation concerning [tuning availability, consistency and durability](/docs/v1.1/operational_guide/availability_consistency_durability).
...
[isolation-group]: /docs/v1.1/operational_guide/placement_configuration#isolation-group
